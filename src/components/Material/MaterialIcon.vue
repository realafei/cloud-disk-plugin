<template>
  <span v-if="svg" :class="['m-svg-icon', 'm-svg-icon--' + name]">
    <svg
      version="1.1"
      ...
      :class="svg.class"
    >
      <defs v-if="svg.defs" v-html="svg.defs"></defs>
      <path v-if="svg.path" fill="currentColor" :d="svg.path" />
      <g v-if="svg.html" v-html="svg.html"></g>
      <slot></slot>
    </svg>
  </span>
  <span v-else class="m-svg-icon"></span>
</template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const style = computed(() => ({
      color: props.color ? props.color : svg.value.fill ? svg.value.fill : null,
    }));
